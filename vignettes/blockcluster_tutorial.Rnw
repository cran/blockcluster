...
blockcluster} and to familiarize its users with its various capabilties. The
...
similated binary data-set. Section \ref{sec:applications} provides two examples
with real data-sets.
...
parameters respectively. The convenient functions {\bf colclusterBinary}, {\bf
...
\item {\bf strategy:} This inpu  can use to control various input parameters. It
...
support (If supported by your Operating system and R). OpenMP will automatically
set the number of threads depending on your system. 
...
The availables binary models are given in the table~\ref{tab:binarymodeltypes}.
\begin{table}[h!]
\centering
...
\hline
 \end{tabular}
\caption{Catgorical models available in package {\bf blockcluster}.}
...
\hline
 \end{tabular}
\caption{Conatinuous models available in package {\bf blockcluster}.}
...
"BCEM" and "BSEM" which are respectively Block EM, Block Classification EM and
Block Stochastic EM algorithms.
...
are "cemInitStep", "fuzzyCemInitStep" and "randomInit". For now only one kind
of initialization exist for every model currently available in the package.
...
Note that you also get the Integrated complete likelihood (ICL) value in case
binary and categorical models. This value can be used for model selection. The
