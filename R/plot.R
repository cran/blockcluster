#' Plot function.
#' 
#' This funtion plot the original and co-clustered datasets.
#' 
#' @param x output object from \code{\link{cocluster}}.
#' @param y Ignored
#' @param ... Additional argument(s) . Currently we support only one additional argument which is "asp". If this
#' is set to FALSE the original aspect ratio is not conserved. In this case, R graphics will display the data
#' in a way that fits the screen and is suitable for viewing. By default "asp" is TRUE.
#' 
...
setMethod(
		...
		definition = function(x, y , ...)
		{
			...
			if (hasArg(asp))
			{
				...
				image(t(data), col=Color, axes=FALSE, zlim=c(min,max), asp=dim(data)[1]/dim(data)[2])
			#,asp =2, c(0.001*dim(data)[2],0.01*dim(data)[1]))
			title("Original Data")
			...
			abline(h=cumsum(rowvec[reverse])[1:x@nbcocluster[1]-1],v=cumsum(colvec)[1:x@nbcocluster[2]-1], col="white",lwd=2)
			
			title("Co-Clustered Data")
			
			#par(mar = c(1,2,1.5,1))
#			image(1, ColorLevels,
#                    matrix(data=ColorLevels, ncol=length(Color),nrow=1),
#                    col=Color,xaxt="n",ylim=c(0,1))
...
setMethod(
		...
		definition = function(x,y,...) {
			...
			x11(xpos=-100,width=12, height=10)
			# get dimension			
