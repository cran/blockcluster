...
threads in you environment (export OMP\_NUM\_THREADS).
...
\hline
     {\bf pik\_rhol\_epsilonkl} & binary & unequal & unequal & CEM/EM\\
     {\bf pik\_rhol\_epsilon} & binary & unequal & equal & CEM/EM\\
     {\bf pi\_rho\_epsilonkl} & binary & equal & unequal & CEM/EM\\
     {\bf pi\_rho\_epsilon} & binary & equal & equal & CEM/EM\\
\hline
 \end{tabular}
...
\hline
     Model  & Datatype & Proportions & Dispersion/Variance & Initialization\\
\hline
     {\bf pik\_rhol\_sigma2kl} & continuous & unequal & unequal & CEM/EM\\
     {\bf pik\_rhol\_sigma} & continuous & unequal & equal & CEM/EM\\
     {\bf pi\_rho\_sigma2kl} & continuous & equal & unequal & CEM/EM\\
     {\bf pi\_rho\_sigma2} & continuous & equal & equal & CEM/EM\\
\hline
 \end{tabular}
...
\hline
     Model  & Datatype & Proportions & Dispersion/Variance & Initialization\\
\hline
     {\bf pik\_rhol\_unknown} & contingency & unequal & N.A & CEM/EM\\
     {\bf pi\_rho\_unknown} & contingency & equal & N.A & CEM/EM\\
...
In the following example, we call the function {\bf coclusterStrategy} without any arguments and
...
One thing which is worth noting in the summary output (above) is that there is
no default value for initialization method. It will be set automatically
depending on the type of input model. To set these input parameters, we have to
...
"BCEM", "BSEM" and "BGibbs" (only for Binary model) which are respectively Block EM, Block Classification EM,
Block Stochastic EM algorithms and Gibbs sampling.
...
are "cemInitStep", "emInitStep" and "randomInit" depending on the data type (See previous Tables).
...
parameters (based on likelihood) among these runs. We call this step {\bf 'xem'} step.
...
and a high number of iterations. We call this step {\bf 'XEM'} step.
...
Note that you also get the explicit Integrated Complete Likelihood (ICL) value in case of
binary and categorical models, and asymptotic value otherwise. This value can be used for model selection. The
